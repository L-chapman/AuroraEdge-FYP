...
**Evidence:**
- `docs/ACADEMIC_NOTEBOOK.md` (problem framing, limitations, rationale)
- `docs/SCAN_ANALYSIS.md` (empirical findings such as adoption rates and common violations)
- Stage documentation and timeline: `docs/Stage_1_README.md` … `docs/Stage_7_README.md`, `docs/DEVELOPMENT_TIMELINE.md`
...
- **Security controls for DNS changes**:
  ...
  - Audit log: `logs/dns_audit.log` (documented in `docs/INTEGRATION_GUIDE.md`).
...
- `docs/INTEGRATION_GUIDE.md` documents secure configuration and operational constraints.
...
**Evidence:**
- Deterministic offline unit tests: `tests/` (pytest) documented in `README.md` and `docs/TESTING_GUIDE.md`.
- Lab setup guidance exists (OpenDMARC/Postfix) in `docs/INTEGRATION_GUIDE.md`.
- Dataset analysis (scan results) exists in `docs/SCAN_ANALYSIS.md` and `reports/`.
...
- **Cloudflare API**: `src/app/dns_fix.py` and `docs/INTEGRATION_GUIDE.md`.
- **OpenDMARC + Postfix**: integration guidance in `docs/INTEGRATION_GUIDE.md` (lab environment).
...
- *What exists:* Production HTTPS deployment guide in `docs/INTEGRATION_GUIDE.md` Section 6.
...
- *What exists:* Empirical comparison methodology in `docs/INTEGRATION_GUIDE.md` Section 7.
...
- **Security measures**: *Token auth + logging met; HTTPS deployment steps are documented in `docs/INTEGRATION_GUIDE.md`*.
- **Automatic fixing**: *Cloudflare fix flow is implemented (CLI flag + dashboard API). Keep claims scoped to DNS-based fixes*.
- **Lab evaluation and tool comparison**: *Lab protocol script exists (`scripts/lab_experiment.py`) and the comparison method is documented in `docs/INTEGRATION_GUIDE.md`*.
