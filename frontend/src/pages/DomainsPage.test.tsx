...
describe('Managed domain incomplete scans', () => {
  ...
  it('labels incomplete scans and rescan notices without trusting provisional grades', async () => {
    ...
    expect(screen.getByRole('heading', { name: '1 managed domain' })).toBeVisible()
    expect(screen.getByText(/does not change DNS or enable scheduled scanning/)).toBeVisible()
    expect(screen.queryByRole('heading', { name: /monitored domain/ })).not.toBeInTheDocument()
